...
<dc:title><xsl:text>Baptism,</xsl:text><xsl:text> </xsl:text><xsl:value-of select="eac-cpf:cpfDescription/eac-cpf:identity/eac-cpf:nameEntry/eac-cpf:part/text()"/></dc:title>
...
<xsl:choose>
  <xsl:when test="$subjects ='baptism'">
 ...
 <dc:subject><xsl:text>Baptism</xsl:text></dc:subject>
</xsl:when>
</xsl:choose>
