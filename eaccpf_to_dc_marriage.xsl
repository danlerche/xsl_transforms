...
<dc:title><xsl:text>Marriage,</xsl:text><xsl:text> </xsl:text><xsl:value-of select="eac-cpf:cpfDescription/eac-cpf:identity/eac-cpf:nameEntry/eac-cpf:part/text()"/></dc:title>
...
<xsl:variable name="bapt" select="eac-cpf:cpfDescription/eac-cpf:description/eac-cpf:biogHist/eac-cpf:chronItem/eac-cpf:date"/>
...
<xsl:choose>
  <xsl:when test="$subjects ='marriage'">
 <dc:date><xsl:value-of select="$bapt"/></dc:date>
 ...
 <dc:subject><xsl:text>Marriage</xsl:text></dc:subject>
</xsl:when>
</xsl:choose>
