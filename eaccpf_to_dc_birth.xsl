...
<xsl:variable name="dob" select="eac-cpf:cpfDescription/eac-cpf:description/eac-cpf:biogHist/eac-cpf:chronItem/eac-cpf:date"/>
...
<xsl:choose>
  <xsl:when test="$subjects ='birth'">
 <dc:date><xsl:value-of select="$dob"/></dc:date>
 ...
 <dc:subject><xsl:text>Birth</xsl:text></dc:subject>
</xsl:when>
</xsl:choose>
